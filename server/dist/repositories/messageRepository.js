...
class MessageRepository {
    async append(record) {
        await database_1.pool.query(`INSERT INTO messages (
    ...
    async findBySharedGroup(sharedGroupId) {
        const result = await database_1.pool.query('SELECT id, chat_id, shared_group_id, author_id, author_key, original_message, rendered_message, posted_at, channel FROM messages WHERE shared_group_id = $1 ORDER BY posted_at ASC', [sharedGroupId]);
